...
// vfs.h). mode 0=read/1=write; the rights the handle gets are fixed at
// open time (RIGHT_READ or RIGHT_WRITE, never both), so e.g. a read-mode
// handle's gt_file_write() call is rejected by the kernel, not just by
// convention. Split out of the former single gui_toolkit.h.
