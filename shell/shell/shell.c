...
#define SHELL_COMMAND_COUNT 107
...
static void shell_commands_init(void) {
    ...
    g_shell_commands[94] = "ring3vfsperm";
    g_shell_commands[95] = "ring3fork";
    g_shell_commands[96] = "ring3guard";
    g_shell_commands[97] = "ring3wait";
    g_shell_commands[98] = "ring3posix2";
    g_shell_commands[99] = "usbinfo";
    g_shell_commands[100] = "ring3signfail";
    g_shell_commands[101] = "ipv6config";
...
void run_command(void) {
    if (streq(g_line_buffer, "help")) {
        ...
        cmd_ring3_users();
    } else if (streq(g_line_buffer, "ring3vfsperm")) {
        cmd_ring3_vfs_perm();
    } else if (streq(g_line_buffer, "ring3fork")) {
        cmd_ring3_fork();
    } else if (streq(g_line_buffer, "ring3guard")) {
        cmd_ring3_guard();
    } else if (streq(g_line_buffer, "ring3wait")) {
        cmd_ring3_wait();
    } else if (streq(g_line_buffer, "ring3posix2")) {
        cmd_ring3_posix2();
    } else if (streq(g_line_buffer, "ring3signfail")) {
        cmd_ring3_signfail();
    } else if (streq(g_line_buffer, "usbinfo")) {
        cmd_usbinfo();
    } else if (streq(g_line_buffer, "wifi")) {
        cmd_wifi();
        ...
        cmd_ipconfig();
    } else if (streq(g_line_buffer, "ipv6config")) {
        cmd_ipv6config();
    } else if (streq(g_line_buffer, "ping6") || starts_with(g_line_buffer, "ping6 ")) {
        cmd_ping6();
    } else if (streq(g_line_buffer, "dns")) {
        cmd_dns();
    } else if (streq(g_line_buffer, "tlsfetch") || starts_with(g_line_buffer, "tlsfetch ")) {
        cmd_tlsfetch();
