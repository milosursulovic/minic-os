...
u16 udp_checksum(u8* src_ip, u8* dst_ip, u8* udp_msg, u16 udp_len) {
    u8 buf[128];
    int i = 0;
    while (i < 4) {
        buf[i] = src_ip[i];
...
void udp_build_header(u8* out, u16 src_port, u16 dst_port, u16 payload_len, u8* src_ip, u8* dst_ip) {
...
// Resolves the target's MAC (milestone 32's real arp_resolve()), builds
// a real Ethernet+IPv4+UDP datagram carrying `payload`, and sends it.
// Real hardware confirmation of the send comes from e1000_send()'s own
...
// Polls (real g_tick_count-bounded, milestone 31/32/33's already-learned
